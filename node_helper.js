 ...
 * or parsed JSON when Accept: application/json is set. Works on Node 12+.
 */
	...
	return new Promise((resolve, reject) => {
		...
		const req = mod.request(opts, (res) => {
			...
			if ([301, 302, 303, 307, 308].includes(res.statusCode) && res.headers.location) {
				...
				httpRequest(res.headers.location, { ...options, _redirects: (options._redirects || 0) + 1 }, postBody)
...
module.exports = NodeHelper.create({
	...
	loadOAuthToken: function () {
		const credPath = path.resolve(
			__dirname,
			this.config.credentialsPath || "credentials/github-oauth.json"
		);
		try {
			...
			Log.error(`MMM-AINews: Failed to load OAuth token from ${credPath}: ${err.message}`);
			Log.error("MMM-AINews: Run 'node modules/MMM-AINews/tools/copilot-auth.js' to authenticate.");
			this.sendSocketNotification("AINEWS_ERROR", {
				error: "No credentials. Run: node modules/MMM-AINews/tools/copilot-auth.js"
			});
		}
	},
	...
	ensureSessionToken: async function () {
		const now = Date.now();
		if (this.sessionToken && (this.sessionExpiresAt - now) > TOKEN_SAFETY_MARGIN_MS) {
			return true;
		}
		...
		for (let attempt = 1; attempt <= 3; attempt++) {
			try {
				...
				this.sessionExpiresAt = expiresAt < 100_000_000_000
					? expiresAt * 1000
					: expiresAt;
				...
				Log.log(`MMM-AINews: Session token acquired (expires in ${Math.round((this.sessionExpiresAt - Date.now()) / 60000)}m), API: ${this.apiBaseUrl}`);
				return true;
			} catch (err) {
				Log.warn(`MMM-AINews: Token exchange attempt ${attempt}/3 failed: ${err.message}`);
				if (attempt < 3) {
	...
	fetchRSS: async function (source) {
		...
		try {
			...
			Log.warn(`MMM-AINews: Failed to fetch/parse RSS from ${source.url}: ${err.message}`);
			return [];
		}
	},
	...
	summarizeSource: async function (source, items) {
		...
		const headlines = truncated.map((it, i) => {
			const desc = it.description ? ` — ${it.description.substring(0, 200)}` : "";
			return `${i + 1}. ${it.title}${desc}`;
		}).join("\n");

		const systemPrompt = "You are a concise news summarizer for a smart mirror display. "
			+ "Produce only the requested paragraph(s). No headings, no bullet points, no markdown. "
			+ "English only. Be factual and concise.";

		const userPrompt = `Here are the latest headlines from "${source.name}":\n\n`
			+ `${headlines}\n\n`
		...
		try {
			...
			const json = await httpJson(`${this.apiBaseUrl}/chat/completions`, {
				method: "POST",
				headers: {
			...
			Log.warn(`MMM-AINews: Summarization failed for "${source.name}": ${err.message}`);
	...
	fetchAndSummarize: async function () {
		...
		Log.log(`MMM-AINews: Updated at ${updatedAt} — ${results.filter((r) => r.summary).length}/${results.length} sources summarized`);
